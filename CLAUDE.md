...
**Startup:** `electron/splash.html` (logo only, on a transparent window: the green tile fades in, then the "M" builds pixel by pixel in stroke order, bottom-left → top-left → middle → top-right → bottom-right) is shown the instant Electron is ready; the main window is built hidden *while* the backend boots, loads the renderer once the probe answers, and is revealed on `ready-to-show` (which closes the splash) — but never before `SPLASH_MIN_MS` after the splash appeared, so a fast start doesn't cut the logo build off. Keep `SPLASH_MIN_MS` in step with the timing vars in `splash.html`. The splash window has the main window's exact bounds (`mainWindowBounds()`), transparent and click-through, so at handoff `flyLogoHome()` can hide the title-bar logo (`[data-app-logo]` in `TitleBar.jsx`), show the main window, call the page's `flyTo()` to shrink the splash logo onto it, then unhide it and close the splash — no window resizing, which flickers on Windows. If the title-bar logo isn't found it falls back to a plain swap; the log says which path ran. The flight must stay compositor-only (the `.mark` transform + `.shadow` opacity): animating the old `filter: drop-shadow` re-rasterized the logo every frame and its `crispEdges` pixels visibly stuttered just before landing; and the main window's slow first frames are kept out of the flight by pre-warming it: while the splash is still building, `reveal()` shows it at opacity 0 and click-through (`prewarmMain()`), and `showMain()` just makes it opaque one frame before the flight — no visible pause (measured: 113 fps with hitches → a steady 180 fps on a 180 Hz display; a 150 ms hold also worked but felt sluggish to the user). Any new code that shows the main window during startup must go through `showMain()`, or a pre-warmed window stays invisible and click-through. **Don't put `-webkit-app-region: drag` back on the splash** — the window is main-window-sized, so it would swallow clicks on the desktop. **The readiness probe must use `127.0.0.1`, never `localhost`**: Electron 28 ships Node 18, which resolves `localhost` to `::1` first, and the backend only listens on 127.0.0.1 — every probe was refused and every launch sat out the full 15 s timeout (the backend itself is up in <1 s). The renderer's own `http://localhost:3001` calls are fine; Chromium falls back to IPv4.
...
- **Origin lockdown (v1.3.3)**: binding to 127.0.0.1 doesn't stop *web pages* in the user's browser from calling `localhost:3001` (multipart uploads and form posts need no preflight), so a global middleware + socket.io `allowRequest` refuse any request whose `Origin` isn't in `ALLOWED_ORIGINS` (`minedash://app`, `http://localhost:5173`, `http://127.0.0.1:5173`) or whose `Host` isn't localhost (DNS rebinding). Requests with **no** Origin (Electron's readiness poll, the game JVM, curl, test scripts) are allowed. The packaged renderer is `file://` — its Origin would be `null` (for fetch: none; for websockets: `file://`), which any site can fake from a sandboxed iframe — so `stampBackendOrigin()` in `electron/main.js` rewrites the Origin of every request to the backend to `minedash://app` via `session.webRequest.onBeforeSendHeaders`. **If you change the dev port, add another front-end origin, or rename `APP_ORIGIN`, update both files** or the app gets 403 on everything. Body-supplied filenames that get joined onto a folder must go through `isUnsafeSegment()` (index.js) / `isSafeFileName()` (launcher.js).
- **Uploads + zip extraction**: every multer upload stages in `DATA_DIR/temp_uploads/` (emptied at boot), never `os.tmpdir()` — the OS temp dir is often on another drive, which turned the final `fs.move` into a multi-GB copy. Map/world imports and Files "Extract here" unpack through `backend/zip-stream.js` (yauzl, streamed disk-to-disk, entries validated for zip-slip + symlinks before anything is written). **Don't use `new AdmZip(path)` for user-supplied archives** — it loads the whole file into memory (a 1 GB map peaked at 3 GB RSS; >2 GB fails).
- **Installing a mod replaces other copies of it**: both `POST /api/servers/:id/mods/install-modrinth` and the launcher's `/profiles/:loader/:version/install` download to `.minedash-update-cache/`, then `modCompat.otherCopiesOf()` finds existing copies (same filename, same Modrinth `projectId` in the metadata, or same primary mod ID in the jar via `readJarModInfo().ownIds`) and `installReplacingCopies()` swaps them into `.minedash-update-backup/`. That's what makes "Change version" a swap instead of leaving two jars (which the loader refuses to start with).
...
- **`TitleBar.jsx`** — custom 38px title bar rendered inside the React app. Uses `style={{ WebkitAppRegion: 'drag' }}` on the container and `WebkitAppRegion: 'no-drag'` on the buttons. Returns `null` when `window.electronAPI?.isElectron` is falsy (dev mode). Contains the MineDash mark (`MineDashLogo.jsx` — pixel "M" on a green tile, also drawn in `public/favicon.svg`, `electron/splash.html` and `electron/assets/icon.ico`; change all four together) and minimize/maximize-restore/close buttons. Close button turns red on hover; others use a muted highlight.
...
**A mod ID is not a Modrinth slug**, and `findAndInstallMissingDeps` must not assume it is. `/project/athena` is a Paper *plugin*; `/project/fusion` is an unrelated mod; `simplytooltips` 404s and Modrinth's search can't match the run-together form. So: a slug hit is only trusted when `projectMatchesTarget()` confirms project type + loader + game version; `modIdQueryVariants()` generates search forms (including a dictionary word-split — `simplytooltips` → `simply tooltips`); `findModrinthCandidates()` ranks them; and each candidate's downloaded jar is verified with `readJarModInfo()` to actually declare the missing ID — deleted and skipped if not. Don't "simplify" this back to a bare slug fetch. These helpers live in `backend/modrinth-resolve.js` (shared with the launcher; they take `{ api, headers }`).

### Mod compatibility: versions, `breaks`, update backups, crash repair (`backend/mod-compat.js`)

**Presence isn't compatibility.** Jars declare version ranges on their dependencies and mods they `breaks`: Sodium 0.8.14 ships `breaks: { iris: "<=1.10.7" }`, Iris 1.10.7 `depends: { sodium: ["0.8.x"] }`. Fabric refuses to start ("Incompatible mods found!") when either is violated. MineDash used to update Sodium to whatever Modrinth called newest (even a beta) and the launcher's crash auto-fix only installed *missing* mods, dropping a second copy beside the broken one — so the game crashed the same way after "fixing" it.

- `backend/mod-versions.js` evaluates ranges the way the loaders do: Fabric/Quilt SemVer predicates (`<=1.10.7`, `0.8.x`, `~`, `^`, arrays = OR) and Maven ranges for Forge/NeoForge (`[1.0,2.0)`; a bare `1.0` matches anything). Every check returns true / false / **null = unknown, treated as OK** — never act on a guess.
- `mod-deps.readJarModInfo(jar, loader, { side })` now also returns `versions`, `ranges`, `breaks` (NeoForge `type = "incompatible"` → breaks; `${file.jarVersion}` resolved from the manifest). `side: 'client'` for launcher instances (mods.toml `side = "CLIENT"` deps apply there); the default `'server'` keeps the old behaviour. `analyzeJars` / `analyzeModsDir` (with virtual `replace` / `add`) list `missing` / `version` / `breaks` problems; compare before/after with `introducedProblems`.
- **Updates are vetted** (`vetUpdates`, both check-updates routes): each candidate jar is downloaded into `.minedash-update-cache/` (reused by the Update step) and checked against the folder; if it introduces a problem, older-but-newer versions are tried; a release user is never offered a beta. The response carries `heldBack[]` per offer and `held[]` for mods with no compatible update. The update routes re-check before swapping (refusing with the reason) and move the old jar to `.minedash-update-backup/` (`backups.json` maps new → previous) instead of deleting it.
- `blocking(problems, newFile)`: a wrong version or a declared break always blocks; a `missing` blocks only when it's *another* mod losing a dependency (the invariant above) — a new requirement of the new jar itself is installed afterwards.
- **Crash repair** (`repairMods`, launcher `runLaunch` close handler and the server exit handler): for each conflict, roll back whichever side has an update backup, else swap the other side for a version that fits, else swap the declaring mod; then install truly missing mods via the validated resolver. The loader's log is only trusted for IDs nothing on disk provides. A `tried` set persists across relaunches so nothing is "fixed" twice in a loop; swapped-out jars go to the backup folder. **Crash repair only touches conflicts the loader's error names** (`crashLog` → `depCrash.loaderErrorSection()`, both mod IDs must appear in it). Without that, a conflict our analysis invents gets "fixed" by swapping out a working mod. That happened in v1.3.3: after correctly rolling a Sodium beta back to 0.8.7, the same run downgraded it to 0.8.0.
- **When several jars provide one ID, the analyzer uses the copy the loader loads** (`loadsInstead` in `analyzeJars`): a top-level jar beats nested jar-in-jar copies, and among nested copies the newest wins. Distant Horizons bundles fabric-api 0.139.4. Taking the first file alphabetically made Sodium look like it "breaks fabric-api <0.140" even with 0.141.6 installed. Launcher bookkeeping (manifest, client extras, modpack records) follows every swap through `renameLauncherMod` / `launcherModHooks`.
...
Updates obey the dependency invariant: before the old jar is swapped out, the new one is checked with `modDeps.analyzeModsDir` + `modCompat.blocking` (see "Mod compatibility" above) — a declared break, a wrong version for another mod, or another mod losing a dependency (a split-out library, a renamed modid) refuses the update for that mod and the old jar stays. After the swap, `restoreRequiredStashedMods` runs and `modDeps.missingDepsOf()` reports what the *updated* jars need but nothing provides, as `missingDeps[]`. The Mods tab offers those through `POST /mods/install-missing`, which intersects the requested IDs with what's actually missing and resolves them via `findAndInstallMissingDeps` (the validated resolver above — never a bare slug fetch).
