...
- **Don't change the release-publish target without updating both ends.** The `publish` block in root `package.json` points electron-builder at `proffesionalprogrammer/minedash-releases` (a public mirror repo used only for distribution). The auto-updater reads from the same place via the generated `app-update.yml`. If you point publishing somewhere else, the updater will silently keep looking at the old location until the next full reinstall.
